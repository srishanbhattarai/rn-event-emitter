...
class Event {
  ...
  static subscribe(eventName, cb) {
    ...
    if (events.hasOwnProperty(eventName)) {
      ...
      if (events[eventName].hasOwnProperty(uuid)) {} // or, 'prop' in obj
  ...
  static emit(eventName, data = {}) {
    ...
    try {
      for (let uuid of Object.keys(registeredListeners)) {
        // ensure we dont use a key that somehow comes from a prototype
        if (registeredListeners.hasOwnProperty(uuid)) {
